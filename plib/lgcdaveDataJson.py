...
def saveData(dataname, datavalue, logit=False):
    ...
    with DataLock:         # prevents two different saveData() at same time
        ...
        try:
            ...
            with open('/home/pi/GoPiLgc/lgcdaveData.json', 'w') as outfile:
...
def getData(dataname=None):
    ...
    try:
        with open('/home/pi/GoPiLgc/lgcdaveData.json', 'r') as infile:
...
def printData():
    print("lgcdaveData.json contents:")
    lData = getData()
    if lData != None:
        for i in lData:
            print("  ",i," : ",lData[i])
    else:
        print("lgcdaveData.json contains no data")
...
def main():
    ...
    if 'chargeCycles' in lData:
        print("removing chargeCycles from lgcdaveData.json")
